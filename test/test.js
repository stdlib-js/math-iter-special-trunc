...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterTrunc, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	it = iterTrunc( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which rounds each iterated value toward zero', function test( t ) {
	var expected;
	var it;
	var N;
	var x;
	...
	N = 101;
	it = iterTrunc( linspace( -2.0, 2.0, N ) );
	t.equal( it.next.length, 0, 'has zero arity' );

	x = linspace( -2.0, 2.0, N );
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = trunc( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which rounds each iterated value toward zero (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': trunc( values[ 0 ] ),
			'done': false
		},
		{
			'value': trunc( values[ 1 ] ),
			'done': false
		},
		{
			'value': trunc( values[ 2 ] ),
			'done': false
		},
		{
			'value': trunc( values[ 3 ] ),
			'done': true
		}
	];
